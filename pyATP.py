...
# Install from pip: pip install fortramformat
#try:
...
class LineConstCards(DataCardStack):
    ''' Stack of cards for a line constants case.
        This is Based on what ATPDraw creates.'''
    def __init__(self):
        conductor_card = DataCard('(I3, F5.4, F8.5, I2, F8.5, F8.5, F8.3, F8.3, F8.3)',
                                 ['IP', 'SKIN', 'RESIS', 'IX', 'REACT', 'DIAM', 'HORIZ', 'VTOWER', 'VMID'])
        end_conductors = DataCardFixedText('BLANK CARD ENDING CONDUCTOR CARDS')
        conductor_cards = DataCardRepeat(conductor_card, end_record = end_conductors,
                                         name = 'conductors')

        DataCardStack.__init__(self, [
                        DataCardFixedText('BEGIN NEW DATA CASE'),
                        DataCardFixedText('LINE CONSTANTS'),
                        DataCardFixedText('$ERASE'),
                        DataCard('(A8,6A6)', ['branch_card', 'in1', 'out1', 'in2', 'out2', 'in3', 'out3']),
                        DataCard('(A80)', ['units']),
                        conductor_cards,
                        DataCard('(F8.2, F10.2, A10, A1, 6I1, A1, 6I1, A1, I1, F8.3, A1, 4I1, I1, A7, I3)',
                                       ['RHO', 'FREQ', 'FCAR', '_1',
                                        'inv_C', 'inv_Ce', 'inv_Cs',
                                        'C', 'Ce', 'Cs', '_2',
                                        'Z', 'Ze', 'Zs',
                                        ...
                                        'ISEG', '_5',
                                        'PUN']),
                        DataCardFixedText('$PUNCH'),
                        DataCardFixedText('BLANK CARD ENDING FREQUENCY CARDS'),
                        DataCardFixedText('BLANK CARD ENDING LINE CONSTANT'),
                        DataCardFixedText('BEGIN NEW DATA CASE'),
                        DataCardFixedText('BLANK CARD')
                    ])
